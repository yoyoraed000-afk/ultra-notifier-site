...
const DEFAULT_PLANS = {
    1: { name: 'Bronze', tier: 1, minValue: 50000000, price: 1.00, slots: 2, minHours: 2, color: '#CD7F32', enabled: true },
    2: { name: 'Silver', tier: 2, minValue: 200000000, price: 2.00, slots: 2, minHours: 2, color: '#C0C0C0', enabled: true },
    3: { name: 'Gold', tier: 3, minValue: 400000000, price: 3.50, slots: 4, minHours: 2, color: '#FFD700', enabled: true },
    4: { name: 'Diamond', tier: 4, minValue: 1000000000, price: 4.25, slots: 2, minHours: 2, color: '#B9F2FF', enabled: true },
    5: { name: 'Diamond Private', tier: 5, minValue: 1000000000, price: 5.00, slots: 1, minHours: 2, color: '#FF00FF', adminOnly: true, enabled: true }
};
...
app.post('/api/admin/plans', requireAdmin, (req, res) => {
    ...
    for (const tier of [1, 2, 3, 4, 5]) {
        ...
        updatedPlans[tier] = {
            ...
            tier: tier,
            minValue: parseInt(newPlan.minValue) || defaultPlan.minValue,
...
app.get('/api/joiner/validate', (req, res) => {
    ...
    if (!user.hwid && hwid) {
        ...
        updateUser(user.id, { hwid: hwid, hwid_history: history, last_active: Date.now() });
    } else {
        updateUser(user.id, { last_active: Date.now() });
    }
...
app.get('/api/joiner/servers', (req, res) => {
    ...
    const plan = PLANS[user.subscription_tier];
    const minValue = plan?.minValue || 0;
    
    // Return servers from database (scanners save here)
    const servers = db.servers || [];
    
    // Filter servers by user's tier minValue
    const filteredServers = servers.filter(s => s.bestValue >= minValue);
